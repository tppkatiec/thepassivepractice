...
chrome.storage?.local?.get(["kajabiInspectorEnabled", "kajabiStickyEnabled"], (data) => {
  toggleInspector.checked = data.kajabiInspectorEnabled || false;
  toggleSticky.checked = data.kajabiStickyEnabled !== false;
  updateStatus();
});

toggleInspector.addEventListener("change", async () => {
  const enabled = toggleInspector.checked;
  chrome.storage?.local?.set({ kajabiInspectorEnabled: enabled });
...
function updateStatus() {
  if (toggleInspector.checked) {
    ...
    statusBox.textContent = "Toggle the inspector, then hover over any element on a Kajabi page.";
